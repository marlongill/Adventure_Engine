...
/*
public class ScrollingTextPanel : Editor
{
    public const int HEADER_HEIGHT = 50;
    public const string GAME_OBJECT_NAME = "ScrollingTextPanel";
    ...
    )
    {
        ...
        if (!string.IsNullOrEmpty(text))
        {
            ...
            titleTMP.text = text;
            titleTMP.color = new Color(1, 1, 0, 1);
    ...
    [MenuItem("GameObject/Custom/Scrolling Text Panel", false, -1)]
    public static void CreatePanel(MenuCommand menuCommand)
    {
        GameObject canvas = menuCommand.context as GameObject;
        ...
        // Top Level Panel
        float panelRight = canvasWidth / 2;
        float panelBottom = canvasHeight / 2;

        GameObject outerPanel = FilledPanel(canvas, GAME_OBJECT_NAME, panelRight, panelBottom, Color.white);

        // Ensure it gets reparented if this was a context click (otherwise does nothing)
        GameObjectUtility.SetParentAndAlign(outerPanel, menuCommand.context as GameObject);

        // Header Panel
        GameObject headerPanel = FilledPanel(outerPanel, "Header", 2, panelBottom - (HEADER_HEIGHT + 4), new Color(0, 0.25f, 0, 255), text: GAME_OBJECT_NAME, fontName: "COUR BD SDF", left:2, top:2);
        ...
        contentTMP.color = new Color(0, 0.75f, 0, 1);
        ...
        Undo.RegisterCreatedObjectUndo(outerPanel, "Create " + outerPanel.name);
        Selection.activeObject = outerPanel;
    }

    [MenuItem("GameObject/Custom/Scrolling Text Panel", true, -1)]
    private static bool CreatePanelValidation()
    {
        if (Selection.activeObject.GetType() == typeof(GameObject))
        {
            GameObject go = (GameObject)Selection.activeObject;
            var component = go.GetComponent("Canvas");
